...
renderer.domElement.addEventListener("dblclick", (e) => {
  ...
  if (intersects.length) {
    ...
    //   )
    //   // .delay(1000) // 1초 딜레이
    //   .easing(TWEEN.Easing.Cubic.In) // 가속도
    //   //.onUpdate(() => render())
    //   .start();
    ...
    new TWEEN.Tween(suzanne.position)
      ...
      .onComplete(() => {
        ...
        new TWEEN.Tween(suzanne.position)
          .to(
            {
              y: p.y + 1, // +1 해주는 이유: 안해주면 중심점이 바로 y = 0 위치로 이동되어 오브젝트가 땅에 꽂혀버림
            },
            250,
          )
          .delay(250)
